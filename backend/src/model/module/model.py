...
class RelGAN(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        generation_C = self.generator([input_B_real, -vector_C2B])  # G(B, v_bc)
        generation_A = self.generator([input_C_real, -vector_A2C])  # G(C, v_ca)
        ...
        generation_alpha = self.generator([input_A_real, vector_A2B * alpha_1])  # G(A,αv)
        generation_cycle_A_back = self.generator([generation_alpha, -vector_A2B * alpha_1])  # G(G(A,αv), -αv)
        ...
        interpolate_alpha = self.interpolate(interpolate_alpha)  # D_interp(G(A, αv)) = interpolated
...
def generate(self, inputs):
    '''
    generate G(x, v)
    :param inputs: x, a(x), and a(y)
    :return: G(x, v)
    '''
    input_A = inputs[0]
    input_A_label = inputs[1]
    input_B_label = inputs[2]
    alpha = tf.cast(inputs[3], tf.float32)  # cast to float
    alpha = tf.reshape(alpha, [-1, 1])  # reshape by (N, 1)
    v = input_B_label - input_A_label

    return self.generator([input_A, v * alpha])
...
class Generator(tf.keras.Model):
    def __init__(self, num_domains, batch_size):
        ...
        self.d1 = Downsample2DBlock(filters=256, kernel_size=(5, 5), strides=2, name_prefix='downsample2d_block1_')
        self.d2 = Downsample2DBlock(filters=256, kernel_size=(5, 5), strides=2, name_prefix='downsample2d_block2_')
...
class Downsample2DBlock(tf.keras.Model):
    '''
    Gated Linear Units and Gated CNN
    given a tensor, do two independent convolutions and get two outputs.
    do sigmoid activation one of them, element-wise multiply two outputs together
    output: h(X) = (X*W+b) * σ(X*V+c)
    '''
    ...
    def __init__(self, filters, kernel_size, strides=1, activation=None, sn=False, name_prefix=None):
        ...
        if sn:
            ...
            self.h1_gates = tf.keras.layers.Conv2D(filters, kernel_size=kernel_size, strides=strides,
                                                   activation=activation, padding='same', name=name_prefix + 'h1_gates')
    ...
    def __call__(self, inputs, training=None, mask=None):
        h1 = self.h1(inputs)
        h1 = self.h1_norm(h1)  # (X*W+b)
        gates = self.h1_gates(inputs)
        gates = self.h1_norm_gates(gates)  # σ(X*V+c)
        h1_glu = self.h1_glu([h1, tf.sigmoid(gates)])  # h(X)
        return h1_glu


class Residual1DBlock(tf.keras.Model):
    '''
    Gated Linear Units and Gated CNN & Residual Connection
    h(X) = (X*W+b) * σ(X*V+c)
    output: (h(X)*U+d) + X
    '''
