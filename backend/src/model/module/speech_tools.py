...
def world_encode_data(wavs, fs, frame_period=5.0, coded_dim=24, n_frames=128):
    '''
    wav -> WORLD -> F0, Sp(->MCEPs), Ap
    :param wavs:
    :param fs:
    :param frame_period:
    :param coded_dim:
    ...
    for wav in tqdm(wavs):
        ...
        if coded_sp.shape[0] < n_frames: continue
        f0s.append(f0)
...
def sample_data_align(dataset_A, nBatch, num_mcep=36, n_frames=128):
    ...
    for i in range(nBatch):
        ...
        # Alignment index 
        idx = np.random.choice(len(dataset_A[atr]))
        
        data_x = dataset_A[atr][idx]
...
def sample_fixed_train(neutral_index, dataset_A, nBatch, num_mcep=36, n_frames=128):
    x = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    x2 = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    y = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    z = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    ...
    for i in range(nBatch):
        labels = np.arange(len(dataset_A))
        x_idx = np.random.choice(len(dataset_A[neutral_index]))
        data_x = dataset_A[neutral_index][x_idx]
        frames_x_total = data_x.shape[1]
        assert frames_x_total >= n_frames
        start_x = np.random.randint(frames_x_total - n_frames + 1)
        end_x = start_x + n_frames
        ...
        if np.random.random() > 0.9995:
            x[i] *= 0
        x_atr.append(neutral_index)

        x2_idx = np.random.choice(len(dataset_A[neutral_index]))
        data_x2 = dataset_A[neutral_index][x2_idx]
        frames_x2_total = data_x2.shape[1]
        assert frames_x2_total >= n_frames
        start_x2 = np.random.randint(frames_x2_total - n_frames + 1)
        end_x2 = start_x2 + n_frames
        ...
        labels = labels[labels != neutral_index]
...
def sample_fixed_align(neutral_index, dataset_A, nBatch, num_mcep=36, n_frames=128):
    x = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    x2 = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    y = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    z = np.zeros((nBatch, num_mcep, n_frames), dtype=np.float32)
    ...
    for i in range(nBatch):        
        # Alignment index
        labels = np.arange(len(dataset_A))
        idx = np.random.choice(len(dataset_A[neutral_index]))
        
        data_x = dataset_A[neutral_index][idx]
        frames_x_total = data_x.shape[1]
        assert frames_x_total >= n_frames
        start_x = np.random.randint(frames_x_total - n_frames + 1)
        end_x = start_x + n_frames
        ...
        data_x2 = dataset_A[neutral_index][idx]
        frames_x2_total = data_x2.shape[1]
        assert frames_x2_total >= n_frames
        start_x2 = np.random.randint(frames_x2_total - n_frames + 1)
        end_x2 = start_x2 + n_frames
